...
let regionSearch = document.createElement('input');
regionSearch.setAttribute('id', "regionSearch");
regionSearch.setAttribute('placeholder', "Region Search");
regionSearch.setAttribute('type', "text");

let btn2 = document.createElement('button');
btn2.setAttribute("id", "regionButton");
btn2.addEventListener("click", function () {
    youtube();
});
btn2.innerHTML = `<button>Search</button>`;

navBar.append(menuDiv, youtubeHead, inputSearch, btn, signIn, threeDots, nineSquares, btn2, regionSearch);
...
async function youtube() {
    ...
    region = document.getElementById("regionSearch").value;
    ...
    var res = await fetch(`https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&regionCode=${region}&key=AIzaSyAHtSLFE9LRsaWu0w3W1RAm1GczN09eVDw&maxResults=20`);
    ...
    items.forEach(({ id, snippet: { title } })=>{
        ...
        videoDiv.innerHTML = `<iframe height="150" src="https://www.youtube.com/embed/${id}" title="YouTube video player"
